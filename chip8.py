...
class Chip8(window.Window):
    ...
    def _6xkk(self):
        ...
        log("Result: " + self.log_x())

    def _7xkk(self):
        # 7xkk ADD Vx, byte. Vx = Vx + kk
        log("7xkk: Vx = Vx +kk\nVx = "+str(self.v[self.x])+"\nkk = " + str(self.nn))
        ...
        log("New Vx = "+str(self.v[self.x]))

    def _8xy0(self):
        set_type = self.instr & 0x000F
        if set_type == 0:
            # 8xy0 Set Vx = Vy.
            log("8xy0: Vx = Vy\n Vx = " + str(self.v[self.x])+"\nVy = " + str(self.v[self.y]))
    ...
    #Draws the screen. Pyglet.window.draw() override
    def draw(self):
        self.clear()
